...
function notifyReservation(r) {
  ...
  const smsText = confirmed
    ? (en ? `Restaurang Demo: your table for ${r.guests} on ${r.date} at ${r.time} is confirmed. Welcome!`
          : `Restaurang Demo: ert bord för ${r.guests} pers ${r.date} kl ${r.time} är bekräftat. Välkomna!`)
    : (en ? `Restaurang Demo: we could not confirm your booking ${r.date} ${r.time}. Please call 000-00 00 00.`
          : `Restaurang Demo: vi kunde tyvärr inte bekräfta er bokning ${r.date} kl ${r.time}. Ring oss gärna på 000-00 00 00.`);
  sendSms(r.phone, smsText);
  if (RESEND_API_KEY && r.email) {
    const subject = confirmed
      ? (en ? 'Table confirmed — Restaurang Demo' : 'Bordsbokning bekräftad — Restaurang Demo')
      : (en ? 'About your booking — Restaurang Demo' : 'Om din bokning — Restaurang Demo');
    const body = JSON.stringify({
      ...
      html: `<div style="font-family:Georgia,serif;max-width:480px;margin:0 auto;color:#26211a">
        ...
        <div style="padding:20px 6px"><p style="font-size:16px">${smsText.replace('Restaurang Demo: ', '')}</p></div>
      </div>`,
    });
    httpsJson('api.resend.com', '/emails', { Authorization: 'Bearer ' + RESEND_API_KEY, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }, body)
